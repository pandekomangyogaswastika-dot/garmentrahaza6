    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(true); setEditItem(null); setForm({ buyer_code: '', buyer_name: '', contact_person: '', phone: '', address: '', email: '' }); }}
                className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:brightness-110 transition" data-testid="add-buyer-btn">
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-[var(--card-surface)] rounded-2xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={handleSave} disabled={!form.buyer_code || !form.buyer_name} className="flex-1 py-2 bg-primary text-white rounded-lg text-sm hover:brightness-110 disabled:opacity-50" data-testid="save-buyer-btn">Simpan</button>
            </div>
          </div>
        </div>
      )}
