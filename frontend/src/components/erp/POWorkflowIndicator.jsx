...
const STEPS = [
  ...
  { key: 'Distributed', label: 'Distributed', color: 'bg-primary' },
          ...
          return (
            <div key={step.key} className="flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${
                done ? 'bg-emerald-500' : active ? step.color : 'bg-secondary'
              }`} title={step.label} />
              {idx < STEPS.length - 1 && (
                <div className={`w-3 h-0.5 ${idx < currentIdx ? 'bg-emerald-400' : 'bg-secondary'}`} />
              )}
            </div>
          );
        })}
          ...
          return (
            <div key={step.key} className="flex flex-col items-center z-10">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${
                  ...
                  : 'bg-[var(--card-surface)] border-border text-muted-foreground'
              }`}>
